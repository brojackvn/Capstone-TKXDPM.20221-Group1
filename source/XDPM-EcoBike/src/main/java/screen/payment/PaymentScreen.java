...
public class PaymentScreen extends BaseScreenHandler {
    ...
    public  PaymentScreen (Stage stage, String path) throws IOException, SQLException {
        ...
        returnButton.setOnMouseClicked(e-> {
            if (Rent.getTimestampList().get(Rent.getTimestampList().size()-1) != null) {
                Rent.addToTimeList(new Timestamp(System.currentTimeMillis()));
            }
...
//                    PopupScreen.success("Payment Successfully");
            ...
            try {
                new SuccessNotificationHandler(stage, Configs.RESULT_SCREEN_PATH).show();
                setScreenTitle("Success Notification");
    ...
    public void setInfo() throws SQLException {
        int rF = new RentBikeController(new CalFee1()).calRentalFee(Rent.getBike().getCategory());
        refund.setText(String.valueOf(new RentBikeController(new CalFee1()).calRentalFee(Rent.getBike().getCategory())) +" VND");
        bikeName.setText(Rent.getBike().getBikeName());
        depositFee.setText(Rent.getDepositFee() + "VND");
        time.setText(Rent.thoigiandathue() + " minutes");
        rentalFee.setText(String.valueOf(Rent.getDepositFee() - new RentBikeController(new CalFee1()).calRentalFee(Rent.getBike().getCategory())) + " VND");
