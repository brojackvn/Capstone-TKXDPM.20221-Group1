...
import screen.notification.ErrorNotificationHandler;
        ...
        submit.setOnMouseClicked(e-> {
            ...
            try {
                if (!depositController.validateName(cardholderName.getText()) || !depositController.validateCardNumber(cardNumber.getText()) || !depositController.validateExpirationDate(expirationDate.getText()) || !depositController.validateSecurityCode(securityCode.getText())) {
                    ...
                    if (paymentController.payRental(depositFees,"DAT COC " + String.valueOf(depositFees) + " VND",securityCode.getText(),cardholderName.getText(),cardNumber.getText(),expirationDate.getText())) {
...
//                        Rent.setStartTime();
                        ...
                        new ErrorNotificationHandler(stage, Configs.ERROR_NOTIFICATION_PATH).show();
                        setScreenTitle("Error Notification");
                    }
                }
            } catch (IOException ex) {
                ex.printStackTrace();
